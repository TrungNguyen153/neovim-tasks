...
---@return table
local function getTargetInfo(codemodel_target, reply_dir)
    return vim.json.decode((reply_dir / codemodel_target['jsonFile'])
        :read())
end
...
local function getCurrentTargetAndExePath()
    ...
    local executablePath = getExecutablePath(buildDir, cmakeConfig.target, getCurrentBuildType(cmakeConfig),
        getReplyDir(buildDir))
