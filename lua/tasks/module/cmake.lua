...
local function purgeBuildDir(module_config, _)
    ...
    if is_windows then
        return {
            cmd = 'cmd',
            args = { '/c', 'rmdir ' .. tostring(build_dir) .. ' /s /q' }
        }
    end
...
local function open_build_dir(module_config, _)
    ...
    if is_windows then
        return {
            cmd = 'cmd',
            args = { '/c', 'start ' .. build_dir.filename }
        }
    end
...
local function run(module_config, _)
    if not module_config.target then
        utils.notify('No selected target, please set "target" parameter. (:Task set_module_param cmake target)',
            vim.log.levels.ERROR)
        return nil
    end
